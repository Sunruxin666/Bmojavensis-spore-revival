...
- `growth_kinetics_group_summary.csv`: group-level phenotype summary used to connect transcriptome and phenotype analyses. The Logistic coefficient `r` is stored as `growth_rate_coef_mean` and `growth_rate_coef_sd`; it is not a maximum-slope estimate.
