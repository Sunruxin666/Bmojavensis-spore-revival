## Reproducible Supplementary Figures S1-S3 and Supplementary Table S3.
##
## Figure contract
## S1: all eight preceding treatment groups are shown for the two secondary
...
group_labels <- c(
  "1g+0" = "1 g + 0 Gy", "1g+6.5" = "1 g + 6.5 Gy",
  "1g+13" = "1 g + 13 Gy", "1g+19.5" = "1 g + 19.5 Gy",
...
plot_kinetics <- function(treatment_label) {
  ...
  display_label <- if (treatment_label == "NaHCO3") expression(NaHCO[3]) else treatment_label
  ggplot(dd, aes(time_h, mean, colour = group, fill = group, group = group)) +
    ...
    labs(x = "Time (h)", y = expression(Relative~OD[600]), title = display_label) +
...
write_csv(qc_table, file.path(out_dir, "RNAseq_count_QC_audit.csv"))
...
plot_pca <- function(obj, label) {
  ggplot(obj$coords, aes(PC1, PC2, colour = dose_f, shape = gravity)) +
    ...
    scale_colour_manual(values = dose_colors, name = "X-ray dose (Gy)") +
    scale_shape_manual(values = gravity_shapes, name = "Gravity condition") +
    labs(
...
## -------------------------------------------------------------------------
## Supplementary Figure S3 and Table S3: individual logistic fits
## -------------------------------------------------------------------------
...
write_csv(
  ...
  file.path(out_dir, "Table_S3_logistic_model_fit_quality.csv")
)
...
# Export every plotted datum for Supplementary Figure S3. The observed rows
# reproduce the gray points, while the fitted rows reproduce the colored
# curves evaluated on the same 201-point grid used by the figure.
s3_source_data <- bind_rows(
  raw_long |>
    transmute(
      SampleID,
  ...
  pred |>
    transmute(
      SampleID,
      Condition = as.character(Condition),
      Dose_Gy = as.numeric(as.character(Dose_f)),
      series = "fitted",
      time_h = Time,
      OD600 = OD_pred
    )
) |>
  arrange(Condition, Dose_Gy, SampleID, series, time_h)
write_csv(
  s3_source_data,
  file.path(out_dir, "Supplementary_Figure_S3_source_data.csv")
)
...
s3 <- ggplot() +
  ...
  facet_grid(
    Condition ~ Dose_f,
    labeller = labeller(
      Condition = c("1g" = "1 g", "SMG" = "SMG"),
      Dose_f = function(x) paste0(x, " Gy")
    )
  ) +
...
message("Supplementary Figures S1-S3, their source data and Table S3 generated successfully.")
