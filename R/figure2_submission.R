...
panel_a_grob <- grid::grobTree(
  grid::rasterGrob(panel_a_raster, interpolate = FALSE),
  # Panel a is a flattened source image. Cover only its two legacy headings;
  # microscopy pixels, dose labels, time labels, and scale bars are untouched.
  grid::rectGrob(x = 0.366, y = 0.895, width = 0.30, height = 0.075,
                 gp = grid::gpar(fill = "white", col = NA)),
  grid::rectGrob(x = 0.737, y = 0.895, width = 0.30, height = 0.075,
                 gp = grid::gpar(fill = "white", col = NA)),
  grid::textGrob("Gravity condition: 1 g", x = 0.366, y = 0.895,
                 gp = grid::gpar(fontfamily = "Arial", fontsize = 8)),
  grid::textGrob("Gravity condition: SMG", x = 0.737, y = 0.895,
                 gp = grid::gpar(fontfamily = "Arial", fontsize = 8))
)
p_a <- wrap_elements(full = panel_a_grob)
