...
anova_caption <- function(fit, metric_name){
  ...
  paste0(metric_name, ': X-ray dose (', fmt_p(p_dose), '), Gravity condition (',
         fmt_p(p_cond), '), X-ray dose × gravity condition (', fmt_p(p_int), ')')
}
...
fit_one_row <- function(row_vals){
  ...
  growth_rate_coef <- if(fit_ok) r else NA_real_
...
# 11. 线性模型（用于图注）
# ============================================================
fit_growth_rate <- lm(growth_rate_coef ~ Dose_f * Condition, data = met2)
fit_lag   <- lm(lag_time ~ Dose_f * Condition, data = met2)

# ANOVA结果用于图注
anova_caption_growth_rate <- anova_caption(fit_growth_rate, 'apparent population-level growth-rate coefficient r')
anova_caption_lag <- anova_caption(fit_lag, 'Lag time')

# Four planned 1 g-SMG contrasts are made within each endpoint. The model-based
# estimates and 95% confidence intervals are retained, and Holm adjustment is
# applied across the four doses to control the endpoint-specific family-wise
# error rate.
star <- function(p) ifelse(p < 0.01, '**', ifelse(p < 0.05, '*', ''))

emmeans_by_dose <- function(fit) {
  contrast_grid <- emmeans(fit, ~ Condition | Dose_f) %>%
    contrast(method = 'pairwise')
  as.data.frame(summary(contrast_grid, infer = c(TRUE, TRUE), adjust = 'none')) %>%
    mutate(
      p_nominal = p.value,
      p_holm = p.adjust(p_nominal, method = 'holm'),
      star_holm = star(p_holm)
    )
}

pair_growth_rate <- emmeans_by_dose(fit_growth_rate)
pair_lag <- emmeans_by_dose(fit_lag)
...
# ============================================================
# 移除NA值
trade_data <- met2 %>% filter(!is.na(growth_rate_coef) & !is.na(inv_lag))
...
pA <- ggplot(traj_summary, aes(Time_h, mean, color = Dose_f, fill = Dose_f, group = Dose_f)) +
  ...
  scale_color_manual(values = col_dose, name = 'X-ray dose [Gy]') +
...
# Dose决定形状（不同剂量不同形状）
# ============================================================
pB <- ggplot(trade_data, aes(growth_rate_coef, inv_lag)) +
  ...
  labs(
    x = expression('Apparent growth coefficient ' * italic(r) ~ '[h'^-1~']'),
    y = expression('1/lag [h'^-1~']')
  ) +
  guides(
    color = guide_legend(title = 'Condition', order = 1),
    shape = guide_legend(title = 'X-ray dose', order = 2)
  ) +
  base_theme +
  theme(legend.position = 'top')

# ============================================================
# 17. Panel C: apparent population-level growth-rate coefficient endpoint
# ============================================================
sum_growth_rate <- met2 %>%
  group_by(Condition, Dose, Dose_f) %>%
  summarise(
    mean = ci_sem(growth_rate_coef)['mean'],
    se = ci_sem(growth_rate_coef)['se'],
    lwr = ci_sem(growth_rate_coef)['lwr'],
    upr = ci_sem(growth_rate_coef)['upr'],
    .groups = 'drop'
  )

# 计算星号位置：图上方的一条水平线
y_max_growth_rate <- max(met2$growth_rate_coef, na.rm = TRUE)
y_range_growth_rate <- y_max_growth_rate - min(met2$growth_rate_coef, na.rm = TRUE)
star_y_growth_rate <- y_max_growth_rate + y_range_growth_rate * 0.15

growth_rate_star <- pair_growth_rate %>%
  transmute(
...
pC <- ggplot() +
  geom_point(data = met2, aes(Dose, growth_rate_coef, color = Condition),
             alpha = 0.4, size = 1.1,
             position = position_jitter(width = 0.1, height = 0, seed = 4002)) +
  geom_errorbar(data = sum_growth_rate, 
                aes(Dose, ymin = lwr, ymax = upr, color = Condition),
                position = position_dodge(width = 0.5), 
                width = 0, linewidth = 0.5, alpha = 0.8) +
  geom_line(data = sum_growth_rate, 
            aes(Dose, mean, color = Condition, group = Condition),
            linewidth = 0.5) +
  geom_point(data = sum_growth_rate, aes(Dose, mean, color = Condition),
             size = 2, shape = 16) +
  geom_text(data = growth_rate_star, aes(Dose, star_y_growth_rate, label = star),
  ...
  coord_cartesian(ylim = c(min(met2$growth_rate_coef, na.rm = TRUE) - y_range_growth_rate * 0.05,
                           star_y_growth_rate + y_range_growth_rate * 0.1)) +
  labs(x = 'X-ray dose [Gy]', 
       y = expression('Apparent growth coefficient ' * italic(r) ~ '[h'^-1~']')) +
  base_theme
...
# 3. Panel B data: apparent population-level growth-rate coefficient and inverse lag
export_data_fig4$panel_B_trade_data <- trade_data

# 4. Panel B回归线预测数据

# 5. Panel C apparent population-level growth-rate coefficient summary
export_data_fig4$panel_C_growth_rate_summary <- sum_growth_rate

# 6. Panel C apparent population-level growth-rate coefficient significance symbols
export_data_fig4$panel_C_growth_rate_stars <- growth_rate_star
...
# 10. ANOVA完整结果
export_data_fig4$anova_growth_rate_full <- as.data.frame(car::Anova(fit_growth_rate, type = 3)) %>%
  mutate(term = rownames(.))
export_data_fig4$anova_lag_full <- as.data.frame(car::Anova(fit_lag, type = 3)) %>%
  mutate(term = rownames(.))

# 11. emmeans成对比较结果
export_data_fig4$emmeans_growth_rate <- pair_growth_rate
