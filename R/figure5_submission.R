...
## 3）基于 KEGG 通路评分展示各重力条件 × X 射线剂量比较的转录响应；
...
##
## 前置要求：
## 需先运行 transcriptome_permanova_audit.R 和 Figure 4 工作流；
## 处理后的 DEG、表型数据与 KEGG 对照表分别位于
## data/transcriptomics/、data/phenotype/ 和 data/annotations/。
##
...
## Table_S1_DEG_list.csv（含时间点和比较元数据的完整差异表达结果）
####################################################
...
term_map <- c(
  ...
  "dose_f"           = "X-ray dose",
  "condition:dose_f" = "Gravity condition × X-ray dose"
)
...
bridge <- deg_burden %>%
  ...
  left_join(
    growth %>% transmute(condition, dose,
                         growth_rate_coef = growth_rate_coef_mean,
                         lag = lag_mean, AUC = AUC_mean),
    by = c("condition", "dose")
  ) %>%
  mutate(
    delta_growth_rate_coef = growth_rate_coef - baseline$growth_rate_coef_mean,
    delta_lag = lag - baseline$lag_mean,
    delta_AUC = AUC - baseline$AUC_mean
  ) %>%
...
pb <- ggplot(pdat,
  ...
  labs(
    ...
    x        = "Gravity condition + X-ray dose [Gy]",
    y        = NULL
  ) +
  theme_journal() +
  theme(
...
ggsave(file.path(out_dir, "Figure5_submission_v16.png"), fig5,
       width = fig_width_cm, height = 14, units = "cm", dpi = fig_res,
       device = ragg::agg_png)
message("Figure 5 main output saved under results/.")
...
message("Figure 5 rewriting-index audit saved under results/.")
...
## 趋势线不含CI，仅展示方向
metric_labs <- c(
  "delta_growth_rate_coef" = expression(Delta * italic(r)),
  "delta_lag"    = expression(Delta * lag),
  "delta_AUC"    = expression(Delta * AUC)
)

bridge_long <- bridge %>%
  pivot_longer(cols = c(delta_growth_rate_coef, delta_lag, delta_AUC),
               names_to  = "metric",
...
message("Figure 5 DEG-burden audit saved under results/.")
...
## Supplementary Table S1: all tested genes in all 14 contrasts.
## Explicit metadata columns distinguish identically named source files at
## 3 h and 10 h and make every row independently interpretable.
table_s1 <- bind_rows(lapply(seq_len(nrow(meta)), function(i) {
  m <- meta[i, ]
  read_csv(m$path, show_col_types = FALSE) %>%
    mutate(
      timepoint = m$timepoint,
      ...
      dose_Gy = m$dose,
      contrast = m$contrast,
      source_file = basename(m$path),
      .before = 1
    )
}))
write_csv(table_s1, file.path(out_dir, "Table_S1_DEG_list.csv"))
...
message("All intermediate tables saved under results/.")
message("\n===== Figure 5 分析完成 =====")
