####################################################
## Figure 7: secondary-spore recovery and challenge responses
##
## Purpose:
## 1) calculate group-level untreated baseline AUC and replicate-level
##    post-challenge net AUC for H2O2 and gentamicin;
## 2) display all treatment groups in the AUC panels and the treatment groups
##    named in the legend in the trajectory panels; and
## 3) describe Pearson associations between selected 10 h functional-category
##    or KEGG scores and post-challenge net AUC within the 1 g + X-ray and
##    SMG + X-ray treatment classes.
##
## Direct inputs:
##   data/secondary_spores/control_1.xlsx and control_2.xlsx
##   data/secondary_spores/h2o2.xlsx and gentamicin.xlsx
##   results/fig5_sig_annotated.csv
##   results/fig56_kegg_pathway_scores.csv
##
## Main outputs:
##   results/Figure7_submission_v18.{pdf,svg,tiff,png}
##   results/Figure7_replicate_AUC_v18.csv
##   results/Figure7_replicate_net_kill_v18.csv
##   results/Figure7_exploratory_correlations_v18.csv
##
## Panel c displays Metabolism, Transport and Iron/Siderophore categories and
## the two-component system, carbon metabolism and phosphotransferase system.
## Each estimable correlation uses three dose-level observations. Constant or
## otherwise non-estimable combinations are recorded as missing.
####################################################
...
message("Panel c 数据合并完成，共 ", nrow(panelD_data), " 行")
...
pB_bar <- plot_auc_replicates(
  net_h2o2, "net_kill",
  expression(paste("H"[2], "O"[2], " response [net AUC]")),
  expression("Post-challenge net AUC")
)
pC_bar <- plot_auc_replicates(
  net_gent, "net_kill", "Gentamicin response [net AUC]", "Post-challenge net AUC"
)
...
#========================
# 8. Panel c：双热图（功能类别 + 精选通路）并排，共用图例
#========================
...
pd_fun <- panelD_data %>%
  ...
  mutate(
    ...
    treat_type = case_when(
      str_detect(group, "^1g\\+")          ~ "1 g + X-ray",
      str_detect(group, "^SMG\\+")         ~ "SMG + X-ray",
      group == "SMG"                       ~ "SMG alone",
      TRUE                                 ~ NA_character_
    )
  ) %>%
...
# 计算功能箱相关性
cor_fun <- pd_fun %>%
  filter(treat_type %in% c("1 g + X-ray", "SMG + X-ray")) %>%
...
fun_grid <- expand.grid(
  ...
  treat_type = c("1 g + X-ray", "SMG + X-ray"),
...
p_left <- make_heatmap(
  ...
  label_col = "label", subtitle = "Functional-category correlations"
) + labs(tag = "c")

p_right <- make_heatmap(
  cor_path_mat,
  x = "x_label", y = "pathway", fill = "r",
  label_col = "label", subtitle = "KEGG-pathway correlations"
)
