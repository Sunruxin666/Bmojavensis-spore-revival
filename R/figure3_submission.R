...
# Reader-facing labels use "Gravity condition" and "X-ray dose" consistently.
# 修改：星号统一标识在图内最上方水平线
...
col_effects <- c('X-ray dose' = '#20854E', 'Gravity condition' = '#0072B5', 
                 'X-ray dose × gravity condition' = '#BC3C29')
...
# Four planned 1 g-SMG comparisons are made within each endpoint. Holm
# adjustment controls the family-wise error rate while retaining the
# endpoint-specific factorial model as the primary analysis.
welch_by_dose <- function(data, endpoint, endpoint_label) {
  out <- lapply(sort(unique(data$Dose)), function(d) {
    smg <- data %>% filter(Condition == 'SMG', Dose == d) %>% pull(.data[[endpoint]])
    one_g <- data %>% filter(Condition == '1g', Dose == d) %>% pull(.data[[endpoint]])
    smg <- smg[is.finite(smg)]
    one_g <- one_g[is.finite(one_g)]
    if (length(smg) < 2 || length(one_g) < 2) {
      return(data.frame(
        endpoint = endpoint_label, Dose = d, estimate_SMG_minus_1g = NA_real_,
        conf_low = NA_real_, conf_high = NA_real_, p_nominal = NA_real_,
        mean_1g = mean(one_g), mean_SMG = mean(smg),
        n_1g = length(one_g), n_SMG = length(smg)
      ))
    }
    test <- t.test(smg, one_g)
    data.frame(
      endpoint = endpoint_label,
      Dose = d,
      estimate_SMG_minus_1g = unname(test$estimate[[1]] - test$estimate[[2]]),
...
decomp_terms <- function(fit, metric){
  ...
  keep %>% transmute(
    ...
    term   = dplyr::recode(term,
                           ...
                           `Dose_f:Condition` = 'X-ray dose × gravity condition'),
...
# ============================================================
# Endpoint-specific planned comparisons with Holm correction.
tests_t50 <- welch_by_dose(g, 't50', 't50')
tests_rel <- welch_by_dose(g, 'rel3h', 'relative germination fraction at 3 h')
...
stars_t50 <- tests_t50 %>%
  transmute(Dose, label = star_holm, y = t50_star_y) %>%
  filter(label != '' & !is.na(y))
...
stars_rel <- tests_rel %>%
  transmute(Dose, label = star_holm, y = rel_star_y) %>%
  filter(label != '' & !is.na(y))
...
# 11-12. Planned 1 g-SMG comparisons with effect estimates, 95% confidence
# intervals, nominal p values and endpoint-specific Holm-adjusted p values.
export_data$ttest_t50_per_dose <- tests_t50
export_data$ttest_rel_per_dose <- tests_rel
...
pD <- ggplot(var_df, aes(x = metric, y = prop, fill = term)) +
  ...
  scale_fill_manual(values = col_effects,
                    breaks = c('X-ray dose', 'Gravity condition', 'X-ray dose × gravity condition')) +
  labs(x = NULL, 
...
cat('图例位置：正上方\n')
cat('Figure terminology standardized: Gravity condition and X-ray dose\n')
