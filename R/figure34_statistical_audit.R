...
fig3_audit <- bind_rows(
  add_adjustments(read_excel(fig3_file, sheet = "ttest_t50_per_dose"), "t50", "p_nominal"),
  add_adjustments(read_excel(fig3_file, sheet = "ttest_rel_per_dose"), "relative germination fraction at 3 h", "p_nominal")
) %>%
...
fig4_fit_qc <- fit_metrics %>%
  mutate(
    ...
    qc_status = case_when(
      ...
      !is.finite(growth_rate_coef) | !is.finite(lag_time) ~ "non-finite derived metric",
...
fig4_emm_audit <- bind_rows(
  add_adjustments(read_excel(fig4_file, sheet = "emmeans_growth_rate"),
                  "apparent population-level growth-rate coefficient r", "p.value"),
